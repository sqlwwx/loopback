...
app.remotes = function(dataSource) {
  var dataSourceName = dataSource ? '_' + dataSource.name: '';
  var remoteName = '_remotes' + dataSourceName;
  if (this[remoteName]) {
    ...
    if (this.get) {
      options = this.get('remoting' + dataSourceName);
    }

    return (this[remoteName]= RemoteObjects.create(options, dataSource));
  }
};
